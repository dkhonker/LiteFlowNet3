...
def correlate(input1, input2, intLevel,signal=0):
    if signal==0:
        if intLevel == 4:
            kernel_size=1
            patch_size=7
        ...
        class Matching(torch.nn.Module):
            ...
            def forward(self, tenFirst, tenSecond, tenFeaturesFirst, tenFeaturesSecond, tenFlow, tenConf):
                if self.confFeat:
                    ...
                    ######fix
                    if self.intLevel == 4:
                        tenCorrelation = torch.nn.functional.leaky_relu(input=correlate(tenFeaturesFirst, tenFeaturesFirst,4), negative_slope=0.1, inplace=False)
                    elif self.intLevel == 3:
                        tenCorrelation = torch.nn.functional.leaky_relu(input=correlate(tenFeaturesFirst, tenFeaturesFirst,3), negative_slope=0.1, inplace=False)
                ...
                tenCorrelation = torch.nn.functional.leaky_relu(input=correlate(tenFeaturesFirst, tenFeaturesSecond,0,1), negative_slope=0.1, inplace=False)
